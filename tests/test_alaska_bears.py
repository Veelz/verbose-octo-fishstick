...
class TestAlaskaBears:
    ...
    def test_get_bear_list(self, api_object):
        logging.log(logging.INFO, '1. Отправить GET-запрос на эндпоинт /bear')
        response = api_object.get_bears_list()
        assert_that(response.status_code, equal_to(200), 'Status code is incorrect')
    ...
    def test_get_bear(self, api_object, create_bear_with_valid_data):
        ...
        logging.log(logging.INFO, f'1. Отправить GET-запрос на эндпоинт /bear/{bear_id}')
    ...
    def test_update_bear(self, api_object, create_bear_with_valid_data):
        ...
        logging.log(logging.INFO, f'3. Отправить PUT-запрос на эндпоинт /bear/{bear_id} с payload шага 1')
        response = api_object.update_bear(bear_model)
        assert_that(response.status_code, equal_to(201), 'Status code is incorrect')
        assert_that(response.text, equal_to(self.SUCCESS_MESSAGE), 'Response body is incorrect')
    ...
    def test_delete_all_bears(self, api_object):
        logging.log(logging.INFO, '1. Отправить DELETE-запрос на эндпоинт /bear')
        ...
        logging.log(logging.INFO, '3. Отправить DELETE-запрос на эндпоинт /bear')
        response = api_object.delete_all_bears()
        assert_that(response.status_code, equal_to(200), 'Status code is incorrect')
        assert_that(response.text, equal_to(self.SUCCESS_MESSAGE), 'Response body is incorrect')

        logging.log(logging.INFO, '4. Отправить GET-запрос на эндпоинт /bear')
        response = api_object.get_bears_list()
        assert_that(response.status_code, equal_to(200), 'Status code is incorrect')
        assert_that(response.json(), empty(), 'Response body is incorrect')
    ...
    def test_delete_bear(self, api_object, create_bear_with_valid_data):
        ...
        logging.log(logging.INFO, f'1. Отправить DELETE-запрос на эндпоинт /bear/{bear_id}')
        ...
        logging.log(logging.INFO, f'3. Отправить DELETE-запрос на эндпоинт /bear/{bear_id}')
        response = api_object.delete_bear(bear_id)
        assert_that(response.status_code, equal_to(200), 'Status code is incorrect')
        assert_that(response.text, equal_to(self.SUCCESS_MESSAGE), 'Response body is incorrect')

        logging.log(logging.INFO, f'4. Отправить GET-запрос на эндпоинт /bear')
        response = api_object.get_bears_list()
        assert_that(response.status_code, equal_to(200), 'Status code is incorrect')
    ...
    def test_get_not_existing_bear(self, api_object, not_existing_bear_id):
        logging.log(logging.INFO, f'1. Отправить GET-запрос на эндпоинт /bear/{not_existing_bear_id}')
        response = api_object.get_bear(not_existing_bear_id)
        assert_that(response.status_code, equal_to(200), 'Status code is incorrect')
        assert_that(response.text, equal_to(self.EMPTY_MESSAGE), 'Response body is incorrect')
    ...
    def test_delete_not_existing_bear(self, api_object, not_existing_bear_id):
        logging.log(logging.INFO, f'1. Отправить DELETE-запрос на эндпоинт /bear/{not_existing_bear_id}')
